            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="card card-info">
                        ...
                        <!-- /.card-header -->
                        <div class="card-body">
                            <form action="doc_file_add_db.php" method="POST" enctype="multipart/form-data" id="AddFile">
                                    ...
                                    <div class="details personal">
                                        <div class="fields">
                                            ...
                                            <div class="input-field mt-2">
                                                ...
                                                <input type="text" class="form-control" name="type_file" aria-describedby="type_file" />
                                            </div>
                                            <div class="input-field mt-2">
                                                <label for="group_file ">กลุ่ม</label>
                                                <input type="text" class="form-control" name="group_file" aria-describedby="group_file " />
                                            </div>
                                            <div class="input-field mt-2">
                                                <label for="name_file ">ชื่อไฟล์</label>
                                                <input type="text" class="form-control" name="name_file" aria-describedby="name_file " />
                                            </div>
                                        </div>
                                    </div>
                                    <button type="submit" class="btn btn-success mt-4">บัททึก</button>
                                </div>
        ...
        <script>
            $(document).ready(function() {
                $("#AddFile").submit(function(e) {
                    e.preventDefault(); // กันไม่ให้ส่งฟอร์มแบบปกติ

                    let formUrl = $(this).attr("action");
                    let reqMethod = $(this).attr("method");
                    let formData = new FormData(this); // ใช้ FormData เพื่อส่งไฟล์ได้
                    ...
                    $.ajax({
                        ...
                        contentType: false,
                        processData: false,
                        dataType: 'json', // บอกว่าเราต้องการข้อมูลกลับมาเป็น JSON
                        success: function(result) {
                            if (result.status == "success") {
                                Swal.fire("สำเร็จ!", result.msg, "success").then(function() {
                                    window.location.href = 'doc_file.php'; // กลับมาหน้าเดิม
                                });
                            } else {
